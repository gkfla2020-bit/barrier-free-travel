...
| 키 | 발급처 | 비고 |
|---|---|---|
| `TOUR_API_KEY` | [공공데이터포털](https://www.data.go.kr/data/15101897/openapi.do) | 자동승인 |
| `TMAP_APP_KEY` | [SK open API](https://openapi.sk.com) | 앱 생성 후 TMAP 사용 신청 |
| `ANTHROPIC_API_KEY` | [Anthropic Console](https://console.anthropic.com) | 유일한 유료 키 |
| `ODSAY_API_KEY` / `TAGO_API_KEY` | [lab.odsay.com](https://lab.odsay.com) / 공공데이터포털 | **없어도 동작** — 도보/"정보 없음" 폴백 |
...
- [ ] **경사 회피** — 경사 데이터 기반 난이도 보강 + 심한 경사 회피 on/off (`slope` 브랜치)
- [ ] **Haiku 온보딩** — 출발지 한 마디("광화문")로 사전 구성 코스 매칭 · 오타 교정("재주"→제주)
- [ ] **모바일 레이아웃** — 시연 영상 녹화용
...
| 데이터 | 출처 |
|---|---|
| 무장애 관광 정보 | 한국관광공사 TourAPI 4.0 「무장애 여행 정보」 (공공누리) |
| 도보 경로 | SK open API — TMAP 보행자 경로안내 |
| 대중교통 · 저상버스 | ODSAY LAB · 국토교통부 TAGO |
