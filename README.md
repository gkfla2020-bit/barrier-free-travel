...
| 용도 | API | 오퍼레이션 | 라이선스 · 비고 |
|---|---|---|---|
| **무장애 장소** (342곳) | [한국관광공사 TourAPI 4.0 「무장애 여행 정보」](https://www.data.go.kr/data/15101897/openapi.do) `KorWithService2` | `locationBasedList2` · `detailCommon2` · `detailWithTour2` | 공공누리 · 자동승인 · **덤프 시 1회만** 호출(런타임 미사용) |
| **도보 경로** (계단 회피) | [SK open API — TMAP 보행자 경로안내](https://tmap-skopenapi.readme.io/reference/%EB%B3%B4%ED%96%89%EC%9E%90-%EA%B2%BD%EB%A1%9C%EC%95%88%EB%82%B4) | `POST /tmap/routes/pedestrian` (`searchOption=30` 계단제외) | Free 1,000건/일 |
| **지도 표시** | SK open API — TMAP 지도 SDK | `/tmap/jsv2` | Free 100,000건/일 · 유일한 프론트 키 |
| **대중교통 경로** | [ODsay LAB](https://lab.odsay.com/) | `searchPubTransPathT` (지하철·버스 길찾기) · `loadLane` (노선 실제 도로 형상) | 등록 앱의 서비스 URI를 Referer로 대조 |
| **저상버스 실시간** | [국토교통부 TAGO](https://www.data.go.kr/data/15098530/openapi.do) `1613000` | `ArvlInfoInqireService/getSttnAcctoArvlPrearngeInfoList` (`vehicletp`) · `BusSttnInfoInqireService/getCrdntPrxmtSttnList` (좌표→정류소) | 공공누리 · 각각 활용신청 필요 · 서울 미커버 |
| **저상버스 (서울)** | 서울시 버스정보시스템 `ws.bus.go.kr` | `stationinfo/getStationByPos` · `arrive/getLowArrInfoByStId` | 구현 완료 · **API 승인 대기** → 미승인 시 "정보 없음" 폴백 |
| **지형 경사** | [Open-Meteo Elevation](https://open-meteo.com/en/docs/elevation-api) | `GET /v1/elevation` (Copernicus DEM GLO-90) | CC BY 4.0 · **API 키 불필요** · 비상업 무료 |
| **코스 추천** | [Anthropic Claude](https://console.anthropic.com) | `claude-sonnet-5` (코스 추천, tool 강제) · `claude-haiku-4-5` (출발지 매칭·오타 교정) | 유일한 유료 API |
...
<sub>**폰트** — 로고: [OK단단체](https://www.okbank.com) (OK저축은행, 무료 배포) · UI: 시스템 폰트 스택(Apple SD Gothic Neo → Pretendard → system-ui)으로 웹폰트 다운로드 없이 렌더</sub>
