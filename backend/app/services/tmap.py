...
# 픽스처는 서울 광화문 데모 경로 — 요청 출발지가 픽스처 시작점에서 이 거리보다 멀면
# 다른 지역이므로 반환 금지 (제주에서 검색했는데 경복궁 경로가 뜨는 오표시 방지)
FIXTURE_MAX_DIST_M = 3000
...
def _difficulty(distance: int, c: dict, prof: dict | None = None,
                avoid_slope: bool = False) -> tuple[str, list[str]]:
    hard, medium = [], []
    if c["stairs"]:
    ...
    # 경사회피 모드에선 경사로 1회부터 '어려움' 요소로 승격 —
    # 사용자가 명시적으로 피하겠다고 한 요소를 '중간'으로 뭉개지 않는다.
    slope_hard = 1 if avoid_slope else SLOPE_HARD
...
def _score(leg: dict, avoid_slope: bool = False) -> dict:
    ...
    이미 slope가 붙어 있으면(재채점) 표고를 다시 조회하지 않는다."""
    if leg["fallback"]:
        # 직선 폴백은 실제 보행로가 아니다. 두 점 사이 표고만 보면 완만해 보이지만
        # 그 사이에 뭐가 있는지 전혀 모르는 상태다 — 여기에 '쉬움'을 매기면
        # 경로 탐색 실패를 '편한 길'로 둔갑시킨다. 폴백의 '어려움'을 그대로 둔다.
        return leg
    prof = leg["slope"] if "slope" in leg else elevation.profile(leg["polyline"])
    leg["slope"] = prof
    level, reasons = _difficulty(leg["distance"], leg["counts"], prof, avoid_slope)
...
def _fetch_leg(start: dict, end: dict, via: dict | None = None,
               opts: tuple = (30, 0)) -> dict:
    """Tmap 호출 + 파싱만 — 채점(_score)은 호출부가 한다.

    opts: 시도할 searchOption 순서. 기본은 계단제외(30) → 실패 시 추천(0).
    경사회피 비교용으로 (0,)만 단독 호출할 수도 있다."""
    body = {
...
def _less_slope(start: dict, end: dict, base: dict) -> dict:
    ...
    best["detour"] = True
    # ①(옵션0 교체)에서 이미 baseline이 잡혀 있으면 '원래 가려던 길'을 그대로 잇는다
    best["baseline"] = base.get("baseline") or {
        "distance": base["distance"], "difficulty": base["difficulty"],
...
def _avoid_slope_leg(start: dict, end: dict, base: dict) -> dict:
    """avoidSlope=True 구간 처리 — 캐시된 기본(base) 경로는 오염시키지 않는다.

    ① 계단제외(30) 경로에 경사로가 1회라도 있으면 일반 추천(0) 경로도 받아
       경사로가 더 적은 쪽을 고른다. 단 계단이 확인된 경로는 절대 고르지 않는다
       (계단 > 경사 — 계단을 만나면 우회 자체가 불가능하다).
    ② 난이도 재채점 — 경사로 1회부터 '어려움' 요소로 승격 (_difficulty avoid_slope).
    ③ 표고(Open-Meteo) 기반 우회 경유점 탐색 (_less_slope)."""
    if base["fallback"]:
        return base
    cand = {**base, "guides": list(base["guides"])}
    _score(cand, avoid_slope=True)  # ② 재채점 — 표고는 base에 붙은 프로파일 재사용
    if cand["counts"]["slope"] >= 1:  # ① 옵션0 비교
        alt = _fetch_leg(start, end, opts=(0,))
        if (not alt["fallback"] and not alt["counts"]["stairs"]
                and alt["counts"]["slope"] < cand["counts"]["slope"]):
            _score(alt, avoid_slope=True)
            alt["guides"].insert(0, f"[경사 회피] 경사로 {cand['counts']['slope']}회 → "
                                    f"{alt['counts']['slope']}회 경로로 대체")
            alt["detour"] = True
            alt["baseline"] = {"distance": cand["distance"], "difficulty": cand["difficulty"],
                               "maxGrade": (cand["slope"] or {}).get("maxGrade", 0),
                               "steepDist": (cand["slope"] or {}).get("steepDist", 0),
                               "polyline": cand["polyline"]}
            cand = alt
    return _less_slope(start, end, cand)  # ③ 표고 우회 (못 찾으면 cand 그대로)
...
def _leg(start: dict, end: dict, avoid_slope: bool = False) -> dict:
    ...
    leg = _avoid_slope_leg(start, end, base) if avoid_slope else base
...
def _fixture_usable(wp: dict, fx: dict) -> bool:
    """픽스처(광화문 데모 경로)를 써도 되는 요청인지 — 출발지가 픽스처 시작점에서
    3km 이내일 때만 True. 다른 지역 요청에 서울 경로를 돌려주는 오표시를 막는다."""
    try:
        lat, lng = fx["legs"][0]["polyline"][0]
        return elevation._haversine(wp["lat"], wp["lng"], lat, lng) <= FIXTURE_MAX_DIST_M
...
def route(waypoints: list[dict], avoid_slope: bool = False) -> dict:
    ...
    # 단, 픽스처는 요청 waypoints와 아무 상관 없는 광화문 데모 경로다.
    # - 출발지가 픽스처 시작점에서 3km를 넘으면(다른 지역) 픽스처를 버리고
    #   직선 폴백 legs를 그대로 내려보낸다 — 제주 검색에 경복궁 경로가 뜨던 버그 수정.
    # - 쓸 때는 fallback=True를 반드시 실어 보낸다. 안 밝히면 FE가 사용자의 코스
    #   이름표를 붙여 진짜 경로인 척 보여주고, 픽스처엔 difficulty가 없어 스키마
    #   기본값 '쉬움'이 찍힌다 — 휠체어 앱에서 최악의 거짓말.
    if all(l["fallback"] for l in legs) and FIXTURE.exists():
        fx = json.loads(FIXTURE.read_text())
        if _fixture_usable(waypoints[0], fx):
    ...
    # 코스 전체 난이도: 최악 구간 + 중간 4개 이상 보정 + 총거리 상한 (반나절 4km)
